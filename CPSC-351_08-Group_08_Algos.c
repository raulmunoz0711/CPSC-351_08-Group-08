...
void runSJF(Player p[], int n) {
    ...
    while (completed < n) {
        int bestIdx = -1, minBurst = 9999; // store index for a player, and find player with smallest burst time
        for (int i = 0; i < n; i++) {
        ...
        // quick if statement to check if no players have arrived yet
        if (bestIdx == -1) { currentTime++; continue; }
        // Process player and calculate timestamps
...
void runPriority(Player p[], int n) {
    int currentTime = 0, completed = 0;
    while (completed < n) {
        int bestIdx = -1, highestPrio = 9999; // placeholders for idx for which player has highest Prio VIP in this case
        for (int i = 0; i < n; i++) {
            if (p[i].arrivalTime <= currentTime && !p[i].isCompleted) {
              // this line below is priority check and putting them in front
